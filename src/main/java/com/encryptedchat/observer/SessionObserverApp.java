...
public final class SessionObserverApp {
    ...
    private static void handleEvent(Map<String, String> e) {
        ...
        switch (type) {
            case "KEY_PAIR_GENERATED":
                ...
                printCrypto(role, "Clave publica EC generada (A = axG)",
            ...
            case "HELLO_SENT":
                ...
                printAttacker("HELLO_SENT [" + role + "]", e.get("bytes"));
                break;

            case "HELLO_RECEIVED":
                printCrypto(role, "Hello recibido",
                        truncate(e.get("bytes"), 60), "del otro extremo");
                printAttacker("HELLO_RECEIVED [" + role + "]", e.get("bytes"));
                break;
            ...
            case "CONFIRMATION_RECEIVED":
                ...
                String validStr = valid
                        ? GREEN + "[OK] CLAVES COINCIDEN - sin MITM detectado" + RESET
    ...
    private static void printEcdhBox(String role, String secret) {
        ...
        System.out.println(BOLD + RED + "  |  [" + role + "] SECRETO ECDH (axbxG)" + pad(38) + "|" + RESET);
        String s = truncate(secret, 52);
        System.out.println(GREEN + "  |  " + s + pad(Math.max(0, 54 - s.length())) + RED + BOLD + "|" + RESET);
        System.out.println(BOLD + YELLOW + "  |  [!] Este valor NUNCA sale por la red" + pad(20) + RED + "|" + RESET);
        System.out.println(BOLD + RED + "  +" + repeat("-", 58) + "+" + RESET);
        System.out.println();
    }
    ...
    private static void printFingerprintBox(String role, String fingerprint) {
        ...
        System.out.println(BOLD + GREEN + "  |  Fingerprint [" + role + "]: "
                + WHITE + fingerprint
                + pad(Math.max(0, 34 - fingerprint.length()))
                + GREEN + "|" + RESET);
    ...
    private static void printAttacker(String label, String bytes) {
        System.out.println(GRAY + "  |  " + label + ": " + truncate(bytes, 40) + RESET);
    }
